...
with open('../data_experimental/execute_circuit/ideal_counts.json', 'r') as fl:
    counts_ideal=json.load(fl)
    
with open('../data_experimental/execute_circuit/noisy_counts.json', 'r') as fl:
    counts=json.load(fl)

list_counts_numerator_noise_free = counts_ideal[(3*(test_qubits+1)):(len(pauli_string_list)*len(noise_scale_range)+(3*(test_qubits+1)))]
list_counts_denominator_noise_free = counts_ideal[(len(pauli_string_list)*len(noise_scale_range)+(3*(test_qubits+1))):((len(pauli_string_list)+1)*len(noise_scale_range)+(3*(test_qubits+1)))]
...
with open('../data_experimental/extrapolation/list_counts_numerator_noise_free.json', 'w') as fl:
    json.dump(list_counts_numerator_noise_free,fl)
with open('../data_experimental/extrapolation/list_counts_denominator_noise_free.json', 'w') as fl:
    json.dump(list_counts_denominator_noise_free,fl)
    
with open('../data_experimental/extrapolation/list_counts_numerator.json', 'w') as fl:
    json.dump(list_counts_numerator,fl)
with open('../data_experimental/extrapolation/list_counts_denominator.json', 'w') as fl:
    json.dump(list_counts_denominator,fl)    
    
new_counts_ideal=counts_ideal[0:(3*(test_qubits+1))]
new_counts=counts[0:(3*(test_qubits+1))]

from benchmark_utils import compute_expectation_from_hamiltonian
ideal_exp=compute_expectation_from_hamiltonian(new_counts_ideal[-1],cost_hamiltonian).real

with open('../data_experimental/original_circuit' + '/ideal_exp.json', 'w') as fl:
    json.dump(ideal_exp,fl)
    
noise_exp=compute_expectation_from_hamiltonian(new_counts[-1],cost_hamiltonian).real
with open('../data_experimental/original_circuit' + '/noise_exp.json', 'w') as fl:
...
noise_exp_measurement_crosstalk=compute_expectation_from_hamiltonian(output,cost_hamiltonian).real
with open('../data_experimental/original_circuit' + '/noise_exp_measurement_crosstalk.json', 'w') as fl:
    json.dump(noise_exp_measurement_crosstalk,fl)
...
with open('../data_experimental/original_circuit' + '/trace_list_measure_all_'+ basis +'.json', 'w') as fl:
...
with open('../data_experimental/original_circuit' + '/trace_list_measure_one_'+ basis +'.json', 'w') as fl:
